    ...
    if max_player:
        ...
        for moves in _3D_moves:
            for i in range(1, len(moves)):
                id = moves[i]
                Easy.move(moves[0], moves[i])

                evaluation = minimax(Easy.matrix, depth - 1, False, id)[0]
                maxEval = max(maxEval, evaluation)
                if maxEval == evaluation:
                    best_move = Easy.matrix

                #idx = moves[i]
                #undo the step
                Easy.move(moves[i], moves[0])
        ...
        for moves in _3D_moves:
            for i in range(1, len(moves)):
                Easy.move(moves[0], moves[i])
                evaluation = minimax(Easy.matrix, depth - 1, True)[0]
