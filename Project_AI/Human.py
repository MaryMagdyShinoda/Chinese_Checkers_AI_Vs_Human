...
def Human():
            ...
            for i in range(0, 17):
                for j in range(0, 25):
                    if matrix[i][j] >= 0:
                        ...
                        pions_rect.append(pygame.draw.rect(screen, colors[int(matrix[i][j])], rect, border_radius=20))
                    if [i, j] in moves:
                        testcircle = pygame.image.load('circle.png')
                        testcircle = pygame.transform.scale(testcircle, (CELL_SIZE, CELL_SIZE))
                        screen.blit(testcircle, (j * CELL_SIZE, i * CELL_SIZE))
    ...
    def show_Position():
            for i in range(0, nb_col):
                for j in range(0, nb_ligne):
                    ...
                    pygame.draw.rect(screen, pygame.Color(255,255,255), rect, width=1)
    ...
    while game_on:
        ...
        if player_index1 == 1 or player_index2 == 5 or player_index3 ==6:
            idx = 1
        elif player_index1 == 2 or player_index2 == 3 or player_index3 == 4:
            idx = 2

        # player turn
        if (winORnot() == False):
            WritingText('Player ' + str(idx) + '\'s Turn', nb_col * CELL_SIZE - 370, nb_ligne * CELL_SIZE - 100, 50, "white")
